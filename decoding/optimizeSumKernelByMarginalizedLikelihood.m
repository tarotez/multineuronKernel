...
% 
function [allParamVec, logLikelihoodDynamics, allParamVecDynamics, safeLoopCnt] = optimizeSumKernelByMarginalizedLikelihood(ks, multiSpikeTrainsBySampleID4optimization, depVar, elemKernelParams, regCoeff, learningRate, loopMax, saveIncrement)
...
while loopCnt <= loopMax
    ...
    R = chol(kernelMat);
    alpha = R \ (R' \ depVar);
    if strcmp(likelihoodType, 'leaveOneOut')
        % invKVec = diag(inv(kernelMat));
        invKVec = diag(inv(R) * inv(R'));
    elseif strcmp(likelihoodType, 'marginaized')
        invK = inv(R) * inv(R');
    end
    ...
    if mod(loopCnt, computeLogLikeStep) == 0
        ...
        elseif strcmp(likelihoodType, 'marginaized')
            logLikelihood = logMarginalizedLikelihoodFromKernelMat(depVar4sgd, kernelMat, invK);
        end
    ...
    elseif strcmp(likelihoodType, 'marginalized')
