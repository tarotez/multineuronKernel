...
% 
function [meanAbsErrors, meanSquaredErrors, est_depVar_vec, test_depVar_vec] = crossValidateKernelWithThinning(totalKernelMatH, totalKernelMatV, depVars, foldNum, regCoeffH, regCoeffV, period, thinConditionsBy)

sampleNum = size(totalKernelMatH,1);

depVar_types = unique(depVars);
...
for foldID = 1:foldNum
    ...
    save temp.for.depVars.mat depVars trainIndices

    
    train_depVar = depVars(trainIndices);
    ...
    train_depVar = depVars(trainIndices_thinned);
    test_depVar = depVars(testIndices);
    % disp(['test_y = ' num2str(test_y')]);
    % disp(['train_y = ' num2str(train_y')]);
    
    if period == 0
        % not angular y        
        trainKernelMat = totalKernelMatH(trainIndices_thinned, trainIndices_thinned);
        testKernelMat = totalKernelMatH(trainIndices_thinned, testIndices);
        alpha = kernelRegression(trainKernelMat, train_depVar, regCoeffH);
        est_depVar = predictByAlpha(testKernelMat, alpha);
    else
        % angular y
        trainKernelMatH = totalKernelMatH(trainIndices_thinned, trainIndices_thinned);
        testKernelMatH = totalKernelMatH(trainIndices_thinned, testIndices);        
        trainKernelMatV = totalKernelMatV(trainIndices_thinned, trainIndices_thinned);
        testKernelMatV = totalKernelMatV(trainIndices_thinned, testIndices);        
        [train_h, train_v] = angle2cartesian(train_depVar, period);
        alpha_h = kernelRegression(trainKernelMatH, train_h, regCoeffH);
        est_h = predictByAlpha(testKernelMatH, alpha_h);
        alpha_v = kernelRegression(trainKernelMatV, train_v, regCoeffV);
        est_v = predictByAlpha(testKernelMatV, alpha_v);
